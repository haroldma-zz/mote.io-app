...
(function() {
  ...
  a.subscribe = function(a) {
    ...
    this.connection.state === "connected" && c.authorize(this.connection.socket_id, function(f, g) {
      f ? c.handleEvent("pusher:subscription_error", g) : b.send_event("pusher:subscribe", {
        channel: a,
        auth: g.auth,
...
(function() {
  Pusher.Util = {
    ...
    flatten: function(b) {
      var c = [],
        a;
      for (a in b) Object.prototype.hasOwnProperty.call(b, a) && c.push([a, b[a]]);
      return c
    },
    any: function(b, c) {
      for (var a = 0; a < b.length; a++) if (c(b[a], a, b)) return !0;
      return !1
    },
    all: function(b, c) {
      for (var a = 0; a < b.length; a++) if (!c(b[a], a, b)) return !1;
      return !0
    },
    method: function(b) {
...
(function() {
  ...
  a.emit = function(a, b) {
    ...
    if (f && f.length > 0) for (c = 0; c < f.length; c++) f[c](b);
...
(function() {
  ...
  d.load = function(b, d) {
    var c = this;
    this.loaded[b] ? d() : (this.loading[b] || (this.loading[b] = []), this.loading[b].push(d), this.loading[b].length > 1 || a(this.getPath(b), function() {
      for (var a = 0; a < c.loading[b].length; a++) c.loading[b][a]();
      delete c.loading[b];
      c.loaded[b] = !0
    }))
  };
  d.getRoot = function(a) {
    var b = Pusher.Util.getDocumentLocation().protocol;
    return (a && a.encrypted || b === "https:" ? this.options.cdn_https : this.options.cdn_http).replace(/\/*$/, "") + "/" + this.options.version
  };
  d.getPath = function(a, b) {
...
(function() {
  function b(a, b) {
    ...
    this.timeout = setTimeout(function() {
      if (c.timeout !== null) b(), c.timeout = null
    }, a)
  }
  var c = b.prototype;
...
(function() {
  ...
  Pusher.Base64 = {
    encode: function(b) {
      return h(b.replace(/[^\x00-\x7F]/g, a))
    }
  }
}).call(this);
(function() {
  ...
  a.send = function(a, b, e) {
    ...
    if (this.script.async === void 0 && document.attachEvent && /opera/i.test(navigator.userAgent)) f = this.options.receiver || "Pusher.JSONP.receive", this.errorScript = document.createElement("script"), this.errorScript.text = f + "(" + a + ", true);", this.script.async = this.errorScript.async = !1;
    var g = this;
...
(function() {
  ...
  c.connect = function(a, b) {
    var c = this,
      ...
      i = null,
      j = function(m, k) {
        k ? b(null, k) : (f += 1, c.loop && (f %= e.length), f < e.length ? (g && (g *= 2, c.timeoutLimit && (g = Math.min(g, c.timeoutLimit))), i = c.tryStrategy(e[f], a, {
          timeout: g,
          failFast: c.failFast
        }, j)) : b(!0))
      },
...
(function() {
  function b(a, b, c, e) {
    ...
    this.options = {
      ...
      hostUnencrypted: e.hostUnencrypted,
      hostEncrypted: e.hostEncrypted
    }
  }
  var c = b.prototype;
...
(function() {
  ...
  c.getQueryString = function() {
    return Pusher.AbstractTransport.prototype.getQueryString.call(this) + "&flash=true"
  };
  ...
  c.createConnection = function(a, b, c, e) {
    var f = this.transport.createConnection(a, b, c, e),
      ...
      k = function(a) {
        ...
        if (a.code === 1002 || a.code === 1003) g.manager.reportDeath();
        else if (!a.wasClean && i && (a = Pusher.Util.now() - i, a < 2 * g.maxPingDelay)) g.manager.reportDeath(), g.pingDelay = Math.max(a / 2, g.minPingDelay)
      };
    f.bind("open", m);
    return f
  };
...
(function() {
  ...
  c.connect = function() {
    var a = this;
    if (!a.connection && a.state !== "connecting") if (a.strategy.isSupported()) if (Pusher.Network.isOnline() === !1) a.updateState("unavailable");
    else {
      ...
      var b = function(c, e) {
          c ? a.runner = a.strategy.connect(0, b) : e.action === "error" ? a.timeline.error({
            handshakeError: e.error
          }) : (a.runner.abort(), a.handshakeCallbacks[e.action](e))
        };
  ...
  c.retryIn = function(a) {
    ...
    a > 0 && b.emit("connecting_in", Math.round(a / 1E3));
